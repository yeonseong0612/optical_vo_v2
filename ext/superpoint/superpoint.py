...
#c_in : 입력 채널수
#c_out : 출력 채널 수
#kernel_size : 합성곱 커널 크기(ex : 3 -> 3 x 3)
#relu : 활성화 함수 사용여부
#padding : 
class VGGBlock(nn.Sequential):
    def __init__(self, c_in, c_out, kernel_size, relu=True):
        ...
        #conv : 2D 합성곱 레이어 정의
        #strid=1, padding을 지정해서 특징 맵 크기가 유지됨
        conv = nn.Conv2d(
...
class SuperPoint(nn.Module):
    default_conf = {
        ...
        "remove_borders": 4,
        "descriptor_dim": 256,                      # 죄종적으로 생성되는 kp desc 차원(256)
        "channels": [64, 64, 128, 128, 256],        # 백본 네트워크에서 각 계층의 채널수
    }
